    ...
    namespace addons {
        ...
        namespace functor {
            ...
            void deformable_im2col(
                ...
                auto num_kernels = p.input_channels * p.output_rows * p.output_cols * p.parallel_imgs;
                ...
                for (auto k = 0; k < num_kernels; k++) {
                    ...
                    const auto current_batch = (k / (p.output_rows * p.output_cols)) % p.parallel_imgs;
                    const auto current_input_channel = k / (p.output_rows * p.output_cols * p.parallel_imgs);
                    const auto current_output_channel = current_input_channel * p.filter_rows * p.filter_cols;

                    const auto group_index = current_input_channel / (p.input_channels / p.offset_groups);

                    auto input_tensor_chipped = input_tensor.chip(current_batch, 0).chip(current_input_channel, 0);
                    EigenTensor<Dtype, 5> offset_tensor_chipped = offset_tensor.chip(current_batch, 0).chip(group_index,
                                                                                                            0);

                    auto column_buffer_tensor_channel = current_output_channel;
                    for (auto current_filter_row = 0; current_filter_row < p.filter_rows; current_filter_row++) {
                        for (auto current_filter_col = 0; current_filter_col < p.filter_cols; current_filter_col++) {
                            auto offset_h = offset_tensor_chipped(current_filter_row, current_filter_col, 0,
                                                                  current_output_row, current_output_col);
                            auto offset_w = offset_tensor_chipped(current_filter_row, current_filter_col, 1,
                                                                  current_output_row, current_output_col);

                            auto mask = Dtype(1);
                            if (use_mask) {
                                EigenTensor<Dtype, 4> mask_tensor_chipped = mask_tensor.chip(current_batch, 0).chip(
                                        group_index, 0);
                                mask = mask_tensor_chipped(current_filter_row, current_filter_col, current_output_row,
                                                           current_output_col);
                            }
            ...
            struct DeformableConv2DFunctor<CPUDevice, Dtype> {
                Status operator()(OpKernelContext *context, typename TTypes<Dtype, 4>::ConstTensor input_tensor,
                    ...
                    auto input_tensor_reshaped = input_tensor.reshape(
                            Shape5D({batches, p.parallel_imgs, p.input_channels, p.input_rows, p.input_cols})
                    );
                    auto filter_tensor_reshaped = filter_tensor.reshape(
                            Shape5D({p.weight_groups, p.output_channels / p.weight_groups, p.filter_channels, p.filter_rows,
                                     p.filter_cols})
                    );

                    auto offset_tensor_reshaped = offset_tensor.reshape(
                            Shape8D({batches, p.parallel_imgs, p.offset_groups, p.filter_rows, p.filter_cols, 2, p.output_rows,
                                     p.output_cols})
                    );
                    ...
                    auto column_buffer_tensor_reshaped = column_buffer_tensor.reshape(
                            Shape3D({p.weight_groups, elems, cols}));
                    ...
                    auto output_tensor_transposed = output_tensor_reshaped.reshape(
                                    Shape5D({batches, p.output_channels, p.parallel_imgs, p.output_rows, p.output_cols})
                            )
                            .shuffle(Shape5D({0, 2, 1, 3, 4}))
                            .reshape(Shape4D({p.input_batches, p.output_channels, p.output_rows, p.output_cols}));
                    ...
                    if (use_bias) {
                        auto bias_tensor_broadcasted = bias_tensor
                                ...
                                .broadcast(Shape4D({p.input_batches, 1, p.output_rows, p.output_cols}));
        ...
        public:
            ...
            void Compute(OpKernelContext *context) override {
                ...
                OP_REQUIRES_OK(context, context->allocate_output(0, input_shape, &input_grad_tensor));
                Tensor *filter_grad_tensor = nullptr;
                OP_REQUIRES_OK(context, context->allocate_output(0, filter_shape, &filter_grad_tensor));
                Tensor *bias_grad_tensor = nullptr;
                OP_REQUIRES_OK(context, context->allocate_output(0, bias_shape, &bias_grad_tensor));
                Tensor *offset_grad_tensor = nullptr;
                OP_REQUIRES_OK(context, context->allocate_output(0, offset_shape, &offset_grad_tensor));
                Tensor *mask_grad_tensor = nullptr;
                OP_REQUIRES_OK(context, context->allocate_output(0, mask_shape, &mask_grad_tensor));
                ...
                mask_grad_tensor->tensor<T, 4>(), column_buffer_tensor.tensor<T, 2>(),
                p);
